...
#####Tool for writing results to disk#########
includegraphics <- function(file, 
                            caption=rep(NA,length(file)),
                            label=rep(NA,length(file)),
                            width=0.8, place="H", page=1) {
  # Wrap a graphics file in LaTex includegraphics text for tex document authoring
  
  # @param file file (with full path) containing graphics
  # @param caption vector of captions of the same length as `file`
  # @param label vector of tex style labels the same length as `file`
  # @param width fraction of textwidth to make included graphic
  # @param place where to place the float
  # @param page page of `file` to include
  # @return character vector, tex includegraphics wrapper
  stopifnot(all(length(file) == length(caption),length(file)==length(label)))
  
  for(i in seq_along(file)) {
    
    writeLines(paste0("\\begin{figure}[",place,"]\\centering"))
    writeLines(paste0("\\includegraphics[width=",width,"\\textwidth, 
                      page=",page,"]{",file[i],"}"))
    if(!is.na(caption[i])) writeLines(paste0("\\caption{",caption[i],"}"))
    if(!is.na(label[i])) writeLines(paste0("\\label{",label[i],"}"))
    writeLines("\\end{figure}") 
  }
